...
def main() -> None:
    ...
    satir("Recall@150", f"{m['recall_at_150']:.3f}  (hedef ≥ 0,90)", m["recall_at_150"] >= 0.90)
    _tavan = 50 / (len(json.loads((ROOT / "data/generated/creators.json").read_text(encoding="utf-8"))) // 30)
    satir("Recall@50", f"{m['recall_at_50']:.3f}  (tavan {_tavan:.2f} — bkz. not)")
