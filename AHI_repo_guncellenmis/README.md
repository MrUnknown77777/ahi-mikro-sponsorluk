...
Yalnızca 23/A uyum katmanını sınamak için (ek bağımlılık gerektirmez):

    python tools/uyum_testi.py

### Ölçüt notu

Aday getirme başarımı **Recall@150** ile raporlanır. Korpusta niş başına 100
ilgili üretici bulunduğundan Recall@50'nin matematiksel tavanı 0,50'dir; ölçüt
bu nedenle havuz büyüklüğüne göre uyarlanmıştır. Her iki değer de
`demo_calistir.py` çıktısında birlikte gösterilir.

### Tarayıcı testleri

`tests/test_prototip.py` uçtan uca arayüz testlerini içerir ve Playwright
gerektirir; `requirements.txt` içinde isteğe bağlı olarak işaretlenmiştir:
